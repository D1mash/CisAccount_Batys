...
namespace Учет_цистерн.Forms.Обработанные_вагоны
{
    public partial class Journal : Form
    {
        ...
        private void LastRenderedService()
        {
            ...
            {
                memoEdit1.Location = new Point(8, 597);
                memoEdit1.Visible = true;
                memoEdit1.Text = "Последняя заявка: " + dt2.Rows[0][1] + " от " + dt2.Rows[0][2] + "" + "\r\n" + "Продукт: " + dt2.Rows[0][5] + "" + "\r\n" + "Была передача: " + dt2.Rows[0][3];
            }
            else
                        ...
                        if (Check != "")
                        {
                            if (textEdit1.Text.Length > 7 && dt.Rows[0][0].ToString() == "PTC Operator TOO")
                            {
                                    ...
                                    (textEdit6.Text != "0" || textEdit7.Text != "0" || textEdit8.Text != "0" || textEdit11.Text != "0" || textEdit25.Text != ""))
                                {
                                    ...
                                    if (checkEdit33.Checked) { DateFromPPS = dateTimePicker7.Value.ToString(); } else { DateFromPPS = "null"; }
                ...
                else if(GroupUpdate == 1)
                {
                    if (textEdit1.Text.Length > 7 && textEdit2.Text == "PTC Operator TOO")
                    {
                                    ...
                                    (textEdit6.Text != "0" || textEdit7.Text != "0" || textEdit8.Text != "0" || textEdit25.Text != "0" || textEdit11.Text != "0"))
                        {
                            ...
                            if (checkEdit33.Checked) { DateFromPPS = dateTimePicker7.Value.ToString(); } else { DateFromPPS = ""; }
        ...
        public void Update(int Id)
        {
            try
            {
                ...
                if (dt.Rows[0][28] == System.DBNull.Value) { dateTimePicker9.Value = System.DateTime.Now; } else { dateTimePicker9.Value = DateTime.ParseExact(dt.Rows[0][28].ToString(), "HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture); }
                if (dt.Rows[0][29] == System.DBNull.Value) { dateTimePicker8.Value = System.DateTime.Now; } else { dateTimePicker8.Value = DateTime.ParseExact(dt.Rows[0][29].ToString(), "HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture); }
                if (dt.Rows[0][30] == System.DBNull.Value) { dateTimePicker6.Value = System.DateTime.Now; } else { dateTimePicker6.Value = DateTime.ParseExact(dt.Rows[0][30].ToString(), "HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture); }
                ...
                if (dt.Rows[0][36] == System.DBNull.Value) { dateTimePicker7.Value = System.DateTime.Now; } else { dateTimePicker7.Value = Convert.ToDateTime(dt.Rows[0][36]); }
        ...
        private void checkEdit33_Properties_CheckStateChanged(object sender, EventArgs e)
        {
            dateTimePicker7.Enabled = (checkEdit33.CheckState == CheckState.Checked);
        }
